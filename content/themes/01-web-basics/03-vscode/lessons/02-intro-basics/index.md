...
VS Code を「コードを書く道具」として日常的に使いこなすための、もっとも頻度の高い操作を整理します。プロジェクトを開いてから保存するまでの一連の流れと、生産性を大きく左右する 5 つのトピックを扱います。
...
VS Code での典型的な開発サイクルは次のようになります。

```
1. プロジェクトを開く（フォルダ / ワークスペース）
   ↓
2. ファイルを開いて編集
   ↓
3. 検索・置換でリファクタリング
   ↓
4. ターミナルでコマンド実行（テスト・ビルド）
   ↓
5. 拡張機能で機能を強化
```

各ステップで使う具体的な操作を、以下の detail で深掘りします。
...
- 日常開発のサイクルは「開く → 編集 → 検索置換 → ターミナル → 拡張で強化」
