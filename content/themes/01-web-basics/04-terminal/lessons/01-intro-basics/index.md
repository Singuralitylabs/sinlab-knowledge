...
ターミナルへの指示は「**コマンド**」と呼ばれます。コマンドは多くの場合、次の3つの部品からできています。
...
| 部品 | 役割 | 例 |
|------|------|-----|
| コマンド | 何をするか（動作そのもの） | `ls`（一覧表示）、`cd`（移動） |
| オプション | どう動かすか（`-` で始まる） | `-l`（詳細表示）、`-a`（隠しファイルも表示） |
| 引数 | 何に対して行うか（対象） | `Documents`（対象のフォルダ） |

**オプション**は、コマンドの動作を細かく調整するためのスイッチです。たとえば `ls` は一覧を表示するコマンドですが、`-l` をつけると詳細表示に、`-a` をつけると隠しファイルも表示する、というように振る舞いが変わります。オプションは `-la` のようにまとめて書くこともできます。

オプションや引数は省略できることも多く、`ls` だけでも動きます。「コマンド単体でまず試す → 必要に応じてオプションを足す」という流れで慣れていきましょう。
