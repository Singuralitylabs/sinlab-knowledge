...
| フィールド | 説明 |
| --- | --- |
| `name` | スキル一覧での表示名。省略するとディレクトリ名が使われます。personal / project ではコマンド名はディレクトリ名由来です（詳細は後述のリファレンス） |
...
| 呼び出し方 | 概要 | 例 |
| --- | --- | --- |
| 手動呼び出し | スラッシュコマンドで明示的に実行。personal / project ではディレクトリ名がコマンド名になり、`name` は一覧の表示名。plugin では `name` がコマンドの最終セグメントになる | `/review` |
...
> [!IMPORTANT]
> `description` の内容が自動呼び出しの精度を決めます。「何をするスキルか」「どんな状況で使うか」を具体的に書くほど、Claude が適切なタイミングで呼び出してくれます。`when_to_use` と合わせた一覧表示は 1,536 文字で切り詰められます。
...
| `effort` | 思考レベルを上書き。`low` / `medium` / `high` / `xhigh` / `max`（利用できる段階はモデルによる） | セッション設定を継承 |
...
| 症状 | 原因と対処 |
| --- | --- |
| スラッシュコマンドとして表示されない | ディレクトリ名が小文字・数字・ハイフンかを確認（plugin では `name` も確認）。`user-invocable: false` になっていないかを見直す |
| 自動呼び出しされない | `description` が抽象的すぎる可能性。「何をするか」「どんなときに使うか」を具体化する。一覧では `when_to_use` と合わせて 1,536 文字で切り詰められる |
