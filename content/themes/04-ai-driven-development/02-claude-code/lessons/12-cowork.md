...
| インターフェース | ターミナル（CLI） | デスクトップアプリ / Web / モバイル / Chrome サイドパネル（チャットに近い UI） |
...
| プロジェクトをチームで共有できない | プロジェクトデータはクラウド同期されず、ローカルフォルダに紐づけたプロジェクトでの Cowork はデスクトップ限定。共有が必要な場合は Chat プロジェクトの活用を検討する |
