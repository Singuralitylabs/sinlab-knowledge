---
title: "Claude Cowork"
description: "Claude Code のエージェント基盤をコーディング以外の知識労働に拡張した Claude Cowork の機能・対応プラン・基本的な使い方を解説。"
order: 12
type: lecture
category: extension
...
> [!NOTE]
> 本記事の内容は **2026 年 8 月時点** のものです。Claude Cowork は一般提供後も Web・モバイル版の展開が進行中であり、対応プラン・機能は今後も変更される可能性があります。実際に利用する際は最新の公式ドキュメントを併せて確認してください。

## このページで学べること

- Claude Cowork とは何か、どのような経緯で提供されてきたか
- 利用できるプランと対応環境（デスクトップ・Web・モバイル）
...
| --- | --- | --- | --- | --- |
| Free | 不可 | — | — | — |
| Pro | 可 | 全機能 | 順次展開中 | 研究プレビューとして利用可 |
| Max | 可 | 全機能 | 先行提供 | 研究プレビューとして利用可 |
| Team | 可 | 全機能 | 先行提供 | 現時点では未対応 |
| Enterprise | 可 | 全機能 + 管理者向け機能（RBAC・支出上限・OpenTelemetry 等） | 先行提供 | 現時点では未対応 |

> [!WARNING]
> **Computer Use は Pro / Max 限定**です（2026 年 8 月時点）。Team / Enterprise では組織展開する場合、Computer Use に依存しない運用（ローカルファイル操作・コネクタ中心）を前提にしてください。

デスクトップは「腰を据えて行う作業」向けにローカルファイル・ブラウザ操作を含む全機能を持ちますが、Web・モバイルのクラウドセッションからローカルファイルや Computer Use を使うには、**デスクトップアプリを開いた状態にしておく必要**があります。
...
| Auto（自動承認、Pro / Max 限定） | Claude が安全性を自己チェックしてから実行する。削除操作は常に確認が入る |
...
| 症状・制限 | 回避策 |
| --- | --- |
| Team / Enterprise で Computer Use が使えない | 現時点は Pro / Max 限定。組織展開ではローカルファイル操作・コネクタ中心の運用に切り替える |
...
- 利用できるのは **有料プラン（Pro / Max / Team / Enterprise）**。対応環境はデスクトップ・Web・モバイル（Web / モバイルはベータで Max 系が先行）
- Claude Code とは役割分担があり、**Dispatch** が開発タスクと知識労働タスクを自動的に振り分ける
