...
最大の利点は、LLM のコンテキストを一切消費しないことです。フォーマッターの実行やログ記録のような定型的な自動化に最適で、コスト面でも予測可能です。
...
エージェントループの随所に発火ポイントが用意されています。ブロック可能なフック（`exit 1` で処理を中断できるもの）と、情報収集や後処理に使うフックに大別できます。

| フック | タイミング | ブロック可能 | 用途例 |
| --- | --- | --- | --- |
| `SessionStart` | セッション開始時 | - | 環境チェック、Git 状態の確認 |
| `UserPromptSubmit` | ユーザーのプロンプト送信時 | ✓ | 入力バリデーション、機密語のフィルタ |
| `PreToolUse` | ツール実行前 | ✓ | 危険なコマンドのブロック、承認判定 |
| `PermissionRequest` | 権限要求時 | ✓ | 自動承認 / 自動拒否の判定 |
| `PostToolUse` | ツール実行後 | - | 編集後の自動フォーマット、lint |
| `Compaction` | コンテキスト圧縮時 | - | 圧縮通知、ログ記録 |
| `SessionEnd` | セッション終了時 | - | サマリ保存、ログのアーカイブ |

### エージェントループ内での発火順序

```text
1. SessionStart       ── セッション開始
2. UserPromptSubmit   ── ユーザー入力
3. PreToolUse         ── ツール実行前（ブロック可）
4. PermissionRequest  ── 権限確認（ブロック可）
5. PostToolUse        ── ツール実行後
6. Compaction         ── コンテキスト圧縮
7. SessionEnd         ── セッション終了
```

> [!NOTE]
> ブロック可能なフックで `exit 1` を返すと、対応する処理（ツール実行やプロンプト処理）は中止されます。`exit 0` を返した場合は通常通り続行します。

## Hooks の設定と実用例

Hooks は `settings.json` の `hooks` フィールドで設定します。各フックタイプに対して、`matcher`（対象ツール名のパターン）と `command`（実行するシェルコマンド）を指定します。`matcher` には `Write|Edit` のようにパイプで複数ツールを OR 指定できます。
...
Claude がファイルを編集（Write または Edit）するたびに Prettier を自動実行してコードスタイルを統一します。`PostToolUse` はブロックしないため、フォーマットに失敗してもエージェントの処理は続行されます。
...
> [!NOTE]
> `$CLAUDE_FILE_PATH` は、フック発火時に対象ファイルのパスが自動的に設定される環境変数です。`$CLAUDE_TOOL_INPUT` などのツール入力も同様に参照できます。
...
> [!WARNING]
> `PreToolUse` で `exit 1` を返すと、そのツール実行は中止されます。逆に `exit 0` を返すと通常通り実行されます。ブロック条件の誤検知を避けるため、パターンは慎重に定義してください。
...
└── mcp/
    └── config.json   # MCP サーバー接続設定
```

| ディレクトリ | 内容 |
| --- | --- |
| `skills/` | 再利用可能なワークフローやナレッジを定義する Markdown ファイル群 |
| `hooks/` | イベント駆動の自動化ルールを定義する設定ファイル |
| `agents/` | 専用のシステムプロンプトとツール制限を持つサブエージェント |
| `mcp/` | 外部サービス接続のための MCP サーバー設定 |
...
典型的な導入コマンドは次のような流れになります（ソースは Git リポジトリやローカルパス、Marketplace のいずれでも指定可能です）。
