...
| `PermissionDenied` | パーミッション拒否時 | - | 拒否ログの記録、通知 |
| `PostToolUse` | ツール実行後 | △ (実行後ブロック) | 編集後の自動フォーマット、lint |
| `PreCompact` | コンテキスト圧縮前 | ✓ | 圧縮対象の調整、通知 |
| `PostCompact` | コンテキスト圧縮後 | - | 圧縮結果の記録 |
| `SessionEnd` | セッション終了時 | - | サマリ保存、ログのアーカイブ |

### エージェントループ内での発火順序（主要なフック）

主要なフックの発火タイミングは次のような順序です（`InstructionsLoaded` や `Stop`、`SubagentStart/Stop` などは省略しています）。
...
6. PreCompact         ── コンテキスト圧縮前
7. PostCompact        ── コンテキスト圧縮後
8. SessionEnd         ── セッション終了
```
