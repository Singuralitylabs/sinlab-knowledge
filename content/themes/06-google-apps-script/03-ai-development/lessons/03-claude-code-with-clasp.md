...
- **実行権限のスコープ**:
  ファイルのプッシュやプル（`push_files` / `pull_files`）は通常の `clasp login` 資格情報で動作します。一方、リモートで関数を実行する（`clasp run-function`）には、Google Cloud プロジェクトの設定や自前 OAuth クライアントによる専用ログイン（`clasp login --creds ...`）、API 実行可能ファイルとしてのデプロイ設定が必要です（詳細はモジュール 3 レッスン 05 で解説）。
- **破壊的変更の抑止**:
