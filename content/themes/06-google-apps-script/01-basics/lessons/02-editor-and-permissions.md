...
ブラウザでエディタを開くと、左側にファイル一覧（`.gs` ファイルや `.html` ファイル）、中央にコードエディタ、上部に実行関数セレクタと「実行」「デバッグ」ボタンが配置されています。下部の実行ログには `console.log` / `Logger.log` の出力が表示されます。

![Apps Script オンラインエディタ：ファイル一覧・コード編集・実行ボタン・実行ログの画面構成](/content-assets/06-google-apps-script/01-basics/images/apps-script-editor.png)
