...
![ボックスモデルの図。外側から margin（外側の余白）→ border（枠線）→ padding（内側の余白）→ content（中身）の順に入れ子になっている](/content-assets/02-web-development-basics/02-css/images/box-model/box-model.svg)
...
![box-sizing の比較。content-box は width に padding と border が加算されて実際の幅が236pxに広がり、border-box は指定した200pxのまま](/content-assets/02-web-development-basics/02-css/images/box-model/box-sizing.png)
