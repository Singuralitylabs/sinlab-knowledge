...
| コンテンツ目次の射影（`lib/content-index.ts`） | `tests/content-index.test.ts` | 目次の不変条件（lecture のみ・detail を出さない・URL 形式・`Lesson.body` を持ち込まない）が壊れやすい | detail 除外、テーマ / モジュール / レッスンの href、件数集計、順序保持、draft フラグ |
