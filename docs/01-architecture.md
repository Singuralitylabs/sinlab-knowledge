...
本サービスの Supabase プロジェクトはポータル（singularity-lab-portal）と共有している。OAuth の `redirectTo` が Redirect URLs の許可リストに無い場合、Supabase Auth はこれを破棄して Site URL（＝ポータル）へフォールバックする。新しいドメインを追加する際は、Redirect URLs に `<domain>/callback` を追加すること。
