...
週インデックスは**エポックからの絶対週番号**を使う。ISO 週番号ではラウンドロビンが崩れる — ISO 年は 52 週または 53 週で、**2026 年は 53 週年**であるため、`isoWeek % N` は年跨ぎでバケットを飛ばしたり二重に選んだりする。
...
3. **`NODE_ENV=production` だと draft が除外される。** 明示せず draft も走査対象にし、レポート側に `draft` フラグを立てる。
...
- HTTP CONNECT プロキシを強制するサンドボックスでは、Bun の `fetch` がプロキシを経由できず**全件 `unknown`** になることがある（`curl` は通る）。リンクではなく環境の問題である。
